...
async function run() {
  try {
    ...
    try {
      core.info("Injecting mitmproxy CA certificate into system trust store...");
      
      const homeDir = os.homedir();
      const mitmCertPath = path.join(homeDir, ".mitmproxy", "mitmproxy-ca-cert.pem");
      const targetCertPath = "/usr/local/share/ca-certificates/mitmproxy.crt";

      // 1. Copy the mitmproxy cert to the Ubuntu cert store
      execSync(`sudo cp ${mitmCertPath} ${targetCertPath}`, { stdio: "ignore" });

      // 2. Update the system CA store
      execSync("sudo update-ca-certificates", { stdio: "ignore" });

      // 3. Export environment variables for tools that don't use the system store automatically
      core.exportVariable("NODE_EXTRA_CA_CERTS", targetCertPath);
      core.exportVariable("SSL_CERT_FILE", "/etc/ssl/certs/ca-certificates.crt");
      core.exportVariable("REQUESTS_CA_BUNDLE", "/etc/ssl/certs/ca-certificates.crt");

      core.info("✅ CA certificate successfully trusted.");
    } catch (certErr) {
      core.warning(`Failed to setup CA certificate: ${certErr.message}`);
    }
